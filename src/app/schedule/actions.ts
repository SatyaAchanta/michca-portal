...
export async function getScheduleGames(
  ...
  const games = await prisma.game.findMany({
    ...
    include: {
      team1: { select: { teamCode: true, teamName: true, teamShortCode: true } },
      team2: { select: { teamCode: true, teamName: true, teamShortCode: true } },
  ...
  return {
    items: games.map((game) => ({
      ...
      awayTeamCode: game.team2.teamCode,
