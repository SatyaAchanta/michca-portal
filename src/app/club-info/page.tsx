...
export default async function ClubInfoPage() {
  ...
  const initialCaptainName = [profile.firstName, profile.lastName].filter(Boolean).join(" ");

  return (
    <div className="bg-background py-12">
      <PageContainer className="space-y-8">
        <div className="space-y-2">
          <h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">Club Info</h1>
          <p className="max-w-2xl text-sm text-muted-foreground sm:text-base">
            Submit captain details for your team assignments. This form is private and can be
            submitted only once.
          </p>
        </div>
        ...
        <Card className="border-primary/35 bg-primary/10 p-5 shadow-sm">
          <p className="text-sm text-muted-foreground sm:text-base">
            Use this form only if you are the current captain. If you need to change a submission
            later, Contact Stats Committee.
          </p>
        </Card>
